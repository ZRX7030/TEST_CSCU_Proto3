...
int cCanSocket::SetCanBittiming(const char * pCanName , int iBitRate)
{
    //socket can setting
    ...
    sprintf(str,"ifconfig %s down",pCanName);
    system( str );

    memset(str,0,sizeof(str));
    sprintf( str, "ip link set %s type can tq %d prop-seg %d phase-seg1 %d phase-seg2 %d sjw %d restart-ms 100",
             pCanName,bt.tq, bt.prop_seg, bt.phase_seg1, bt.phase_seg2, bt.sjw );
    system( str );    

    return bt.bitrate;
}

int cCanSocket::SocketCan_Init(const char *pCanName, int iBitRate)
{
    int loopback = 0; /* 0 = disabled, 1 = enabled (default) */
    struct sockaddr_can addr;
    struct can_frame g_canFrame;
    struct ifreq ifr;
    memcpy(chCanNameAttr,pCanName,strlen(pCanName));
    iBitRateAttr = iBitRate;

    char szTemp[256] = {0};
    int ret;
    //struct can_filter filter;
    sprintf(szTemp,"ifconfig %s down",pCanName);
    system( szTemp);
...
//    filter[1].can_mask = 0x700;
    struct can_filter filter;
    filter.can_id = 0xfa00;
    filter.can_mask = 0xff00;
    setsockopt(g_canHandle,SOL_CAN_RAW,CAN_RAW_FILTER,&filter,sizeof(filter));
    ...
    SetCanBittiming( pCanName, iBitRate);
    sprintf(szTemp,"ifconfig %s up",pCanName);
    system(szTemp);

    return 1;
}

int cCanSocket::SocketCan_Close()
{
    if(g_canHandle>0)
    {
        close(g_canHandle);
...
int cCanSocket::SocketCan_Read(struct can_frame *frame)
{
    int nbytes, nfds;
    fd_set rfds;
    struct timeval tv;
...
int cCanSocket::SocketCan_Write(struct can_frame *frame)
{
    int nbytes;
