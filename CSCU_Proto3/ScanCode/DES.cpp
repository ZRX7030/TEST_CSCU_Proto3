...
//            int j = padlen>>3;
    ...
    {
        unsigned char   cvec[8] =  {'q','d','t','e','l','d','_','1'}; //扭转向量
...
void cDES::ProcDES(InfoMap qmap)
{
    unsigned char tmp_buf_key[8] = {'T','E','L','D','2','2','T','P'};
    //组合密匙
    tmp_buf_key[4] = qmap[Addr_ScanCode].at(4);
    tmp_buf_key[5] = qmap[Addr_ScanCode].at(7);
    tmp_buf_key[6] = qmap[Addr_ScanCode].at(20);
    tmp_buf_key[7] = qmap[Addr_ScanCode].at(21);
    QString str_key = QString(QByteArray(qmap[Addr_ScanCode]).data());
    str_key.remove(0,5);
    str_key.remove(2,1);
    str_key.remove(14,2);
    //DES解密
    RunDes(1,1,(const unsigned char *)QByteArray::fromBase64(str_key.toAscii()).data(),outstr,str_key.toAscii().length(),tmp_buf_key,8);

    emit sendDES((char *)outstr);
}
