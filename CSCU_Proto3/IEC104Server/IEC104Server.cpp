...
CIEC104Server::CIEC104Server()
{
	...
	m_bEmergency = false;
	m_bExit = false;
	m_cServerType = SERVER_REMOTE;
	m_iHostIndex = -1;
	m_iTryConnect = 0;

	m_pSetting->querySetting(&m_confServer, m_cServerType == SERVER_REMOTE ? PARAM_SERVER0 : PARAM_SERVER1);
	//转换站地址
 ...
 * 返回值	无
 */
void CIEC104Server::slot_onThreadRun()
{
 ...
 * 返回值	无
 */
void CIEC104Server::slot_onConnected()
{
 ...
 * 返回值	无
 */
void CIEC104Server::slot_onDisconnected()
{
 ...
 * 返回值	无
 */
void CIEC104Server::slot_onError(QAbstractSocket::SocketError err)
{
 ...
 * 返回值	无
 */
void CIEC104Server::slot_onReceive()
{
...
//能效计划临时用
void CIEC104Server::parseBatteryStatus(InfoMap &mapInfo)
{
    batteryInfoMap = mapInfo;
}
void CIEC104Server::parsePowerOptimizerStatus(InfoMap &mapInfo)
{
 ...
 * 返回值	无
 */
void CIEC104Server::slotFromBus(InfoMap mapInfo, InfoAddrType type)
{
    QIteratorList list;
	QByteArray arData;
	uchar cCmd = 0, cRet = 0;
    ...
    if(!m_bConnected || !m_bActived){
		if(type == AddrType_Comm_Addr){//解析通信地址
			ParseCommAddr(mapInfo);
		}
 ...
 * 返回值	无
 */
void CIEC104Server::timerEvent(QTimerEvent *event)
{
	if(event->timerId() == m_iServerTimer){
		...
		switch(m_ServerState){
			case DATA_CHECK://数据有效性检查阶段，超时时间60秒，完成后切换至地址获取状态
				WriteLog(QString().sprintf("Wait for data valid %d", m_iServerTimeout), 1);
				if(m_pFilter->isAllValid() || m_iServerTimeout <= 0){
					m_ServerState = GET_ADDRESS;	
					m_iServerTimeout = 0;
				}
				break;
			case GET_ADDRESS:
				WriteLog(QString().sprintf("Get net address %d", m_iServerTimeout), 1);
				if(m_iServerTimeout <= 0){
					m_iServerTimeout = WAIT_ADDR_TIME;
 ...
 * 返回值  成功返回0，有错误发生返回-1
 */
int CIEC104Server::ParseFrame(uchar *pszData, int iLen, InfoMap& mapInfo, InfoAddrType& type)
{
    Net_Hdr* pNetHdr;
 ...
 * 返回值 	无	
 */
void CIEC104Server::SendKeyAgreement()
{
	Frame_Net stFrame;
 ...
 */
bool CIEC104Server::ParseKeyAgreement(uchar* pszData, int iLen)
{
	Frame_Net stFrame;
	int iEncrypt;
    uchar psR1R2[128];
 ...
 * 返回值 	成功返回true，失败返回false	
 */
bool CIEC104Server::ParseFrameU(uchar* pszData, int iLen)
{
 ...
 * 返回值 	无
 */
void CIEC104Server::SendFrameU(uchar cCmd)
{
	Frame_U stFrame;
	int iLen;
 ...
 * 返回值 	成功返回true，失败返回false	
 */
bool CIEC104Server::ParseFrameI(uchar* pszData, int iLen, InfoMap& mapInfo, InfoAddrType& type)
{
	APCI* pApci;
	ASDU_Hdr* pAsduHdr;
    QIteratorList list;
 ...
 * 返回值		无
 */
void CIEC104Server::SendFrameI(QIteratorList& list, uchar cFrameType, ushort sReason, int iSerial)
{
	Frame_I stFrame;
	InfoMap::iterator it;
	QByteArray ar;
 ...
 * 返回值	无
 */
void CIEC104Server::SendEncryptData(uchar *pszData, int iLen)
{
	Frame_Net stFrame;
    int iEncrypt;
 ...
 */
bool CIEC104Server::ParseInfoTag(uchar* pszData, uint iLen, uchar cLimit, InfoMap& mapInfo)
{
	QByteArray arData;
 ...
 */
bool CIEC104Server::Parse104Frame(uchar* pszData, int iLen, InfoMap& mapInfo, InfoAddrType& type)
{
	APCI* pApci;
	uchar szBuff[512];
	int iDecrypt = 0;
 ...
 * 返回值	无
 */
void CIEC104Server::SendQueryAll(QueryType type)
{
    QIteratorList list;
	InfoMap mapInfo;
	QByteArray arData;
 ...
 */
bool CIEC104Server::QuerySignal(uchar cCanAddr, int& iInfoAddr, QByteArray& arData)
{
	TerminalStatus stStatus;
	memset(&stStatus, 0, sizeof(TerminalStatus));
 ...
 */
/*
bool CIEC104Server::QueryBatteryStatus(uchar cCanAddr, int& iInfoAddr, QByteArray& arData)
{
    iInfoAddr = 0xDA000A;
...
/*
bool CIEC104Server::QueryBatteryEleInfo(uchar cCanAddr, int& iInfoAddr, QByteArray& arData)
{
    //uchar cQuality = 0;
...
bool CIEC104Server::QueryPowerOptimizerStatus(uchar cCanAddr, int& iInfoAddr, QByteArray& arData)
{
    iInfoAddr = 0xDC0001;
 ...
 * 返回值	有效数据返回true，无效返回false
 */
bool CIEC104Server::QueryMeasure(uchar cCanAddr, int& iInfoAddr, QByteArray& arData)
{
	TerminalStatus stStatus;
	QByteArray ar;
	uchar cQuality = 0;
 ...
 * 返回值	有效数据返回true，无效返回false
 */
bool CIEC104Server::QueryMeasure2(uchar cCanAddr, int& iInfoAddr, QByteArray& arData, bool bCharge)
{
	TerminalStatus stStatus;
	int iMeasure2 = 0;
	uchar cQuality = 0; 
 ...
 * 返回值	无
 */
void CIEC104Server::SendSignal(uchar cCanAddr, bool bBurst)
{
    QIteratorList list;
	InfoMap map;
	QByteArray arData;
 ...
 * 返回值	无
 */
void CIEC104Server::SendMeasure(uchar cCanAddr)
{
    QIteratorList list;
	InfoMap map;
	QByteArray arData;
 ...
 * 返回值	无
 */
void CIEC104Server::SendMeasure2(uchar cCanAddr, bool bBurst)
{
    QIteratorList list;
	InfoMap map;
	QByteArray arData;
 ...
 */
bool CIEC104Server::IsBurstMeasure2(InfoMap& mapInfo)
{
	TerminalStatus stStatus;
	memset(&stStatus, 0, sizeof(TerminalStatus));
 ...
 * 返回值	无
 */
void CIEC104Server::SendBMS(uchar cCanAddr)
{
    QIteratorList list;
	InfoMap map;
 ...
 * 返回值	无
 */
void CIEC104Server::SendHighVolate()
{
    QIteratorList list;
	InfoMap map;
	QByteArray ar, arData;
...
void CIEC104Server::SendAmmeterDataType(InfoMap mapInfo,QIteratorList list)
{
    int cEnergyDataType;
...
///
/// \brief CIEC104Server::SendAmmeterData
///召唤电表数据
void CIEC104Server::SendAmmeterDataType1(InfoMap mapInfo,QIteratorList list)
{
    QByteArray arData;
    int iMeasure = 0;
    uint uiEnergy = 0;
...
///
/// \brief CIEC104Server::SendAmmeterDataType2
///召唤电表数据-整点冻结电能数据   add by zrx 2017-03-24
void CIEC104Server::SendAmmeterDataType2(InfoMap mapInfo,QIteratorList list)
{
    QByteArray arData;
    uint uiEnergy = 0.0;
    if(mapInfo.empty()){
...
void CIEC104Server::SendAmmeterDataType3(InfoMap mapInfo,QIteratorList list)
{
    QByteArray arData;
    QDateTime dt;
    int iMeasure = 0;
...
void CIEC104Server::SendAmmeterDataType4(InfoMap mapInfo,QIteratorList list)
{
    QByteArray arData;
    int iMeasure = 0;
    float fEnergy = 0.0;
 ...
 * 返回值	无
 */
void CIEC104Server::SendVersion()
{
    QIteratorList list;
	InfoMap map;
	QByteArray arData;
 ...
 * 返回值	无
 */
void CIEC104Server::SendModuleVersion(uchar cCanAddr)
{
    QIteratorList list;
	TerminalStatus stStatus;
	InfoMap map;
 ...
 * 返回值	无
 */
void CIEC104Server::SendStationEnv()
{
    QIteratorList list;
	InfoMap map;
	QByteArray ar, arData;
 ...
 * 返回值	无
 */
void CIEC104Server::SendFrozenEnergy(InfoMap& mapInfo)
{
    QIteratorList list;
	InfoMap map;
...
void CIEC104Server::SyncTime(InfoMap& mapInfo)
{
    QIteratorList list;
    InfoMap map;
    QByteArray arData;
 ...
 * 返回值 	无
 */
bool CIEC104Server::SetAmmeterParam(uchar* pszData, int iLen)
{
	stAllAmmeterConfig conf;
 ...
 * 返回值 	无
 */
bool CIEC104Server::SetAmmeterParamSet(InfoMap& mapInfo)
{
    stAllAmmeterConfig conf;
    stAmmeterConfig stAmmeter;
    QByteArray arrData;
 ...
 * 返回值		无
 */
void CIEC104Server::SetNetState(bool bConnected, QString strError)
{	
	QString strSql;
 ...
 * 返回值		无
 */
void CIEC104Server::SendNetState(bool bOffline)
{
	InfoMap mapInfo;
 ...
 * 返回值		无
 */
void CIEC104Server::WriteEmergencyEnable(InfoMap &mapInfo)
{
 ...
 * 返回值		无
 */
void CIEC104Server::WriteEmergencySetting(InfoMap &mapInfo)
{
	EmergencyConfig config;
   	QIteratorList list;
	uchar cRet = 0x01;
 ...
 * 返回值		无
 */
void CIEC104Server::WriteQueueGroupInfo(InfoMap &mapInfo)
{
	QIteratorList list;
	InfoMap map;
	QByteArray ar;
 ...
 * 从服务器列表中取出地址，进行连接
 */
bool CIEC104Server::connectToServer()
{
	if(m_strHost.isEmpty() || m_sPort <= 0)
		return false;
...
/*
bool CIEC104Server::GetHostList(stServerListConfig &confServerList)
{
	HostNode node;
...
bool CIEC104Server::WriteServerList(InfoMap &mapInfo)
{
	uint addr = 0;
	uchar cRet = 0x01;
 ...
 * 返回值		无
 */
void CIEC104Server::WriteChargeGunGroupInfo(InfoMap &mapInfo)
{
    QIteratorList list;
    InfoMap map;
    QByteArray ar;
 ...
 * 返回值	无
 */
void CIEC104Server::WriteLog(uchar* pszData, int iLen, uchar cType, bool bSend)
{
    QString str, strLog;
    uchar* p;
	int iPos = 0;
 ...
 * 返回值	无
 */
void CIEC104Server::WriteLog(QString strLog, int iLevel)
{
	//int iType = m_cServerType == SERVER_REMOTE ? LOG_MODULE_SERVER_0 : LOG_MODULE_SERVER_1;
 ...
 * 返回值 	0表示无错误，-1表示有错误
 */
int CIEC104Server::InitModule(QThread* pThread)
{
 ...
 * 返回值 	0表示无错误，-1表示有错误
 */
int CIEC104Server::RegistModule()
{
	QList<int> list;
	...
	list.append(AddrType_Comm_Addr);//通信地址获取
 ...
 * 返回值 	0表示无错误，-1表示有错误
 */
int CIEC104Server::ModuleStatus()
{
	return 0;
}
...
{
	CModuleIO* pModule = new CIEC104Server();
 ...
 * 返回值	无
 */
void CIEC104Server::SendFrozenEnergyPeakShaving(InfoMap& mapInfo)
 ...
 * 返回值		无
 */
int  CIEC104Server::CheckChargeGunGroupInfo(InfoMap &mapInfo)
{
 ...
 *主动防护告警信息
 */
void CIEC104Server::SendActiveDefendAlarm(InfoMap &mapInfo)
{
    QIteratorList list;
...
void CIEC104Server::ParseCommAddr(InfoMap &mapInfo)
{
	...
	if(mapInfo.contains(Addr_Comm_Addr)){
		list = mapInfo[Addr_Comm_Addr].split(':');
		if(list.count() >= 2){
			m_strHost = list.at(0);	
			m_sPort = list.at(1).toInt();
		}
	}
	...
	WriteLog(QString("Get Address Success Host=%1 Port=%2").arg(m_strHost).arg(m_sPort));
}
