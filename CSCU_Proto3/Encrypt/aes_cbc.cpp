...
int AES_cbc_encrypt(const unsigned char *in, unsigned char *out, size_t len, unsigned char *key, unsigned char *iv, int enc)
{
	...
	if(enc){
		if(AES_set_encrypt_key(key, 128, &k) < 0)
			return -1;

		padding = AES_BLOCK_SIZE - (len % AES_BLOCK_SIZE);

		buff = (unsigned char *)malloc(len + padding);
		memcpy(buff, in, len);
		memset(buff + len, padding, padding);

		AES_cbc128_encrypt(buff, out, len + padding, &k, iv);	

		free(buff);
		return len + padding;
	}else{
		if(AES_set_decrypt_key(key, 128, &k) < 0)
			return -1;
	
		AES_cbc128_decrypt(in, out, len, &k, iv);	
		for(int i = len - AES_BLOCK_SIZE; i < len; i++){
			if(out[i] > 0 && out[i] <= AES_BLOCK_SIZE){			
				out[i] = '\0';
				return i;
			}
		}
