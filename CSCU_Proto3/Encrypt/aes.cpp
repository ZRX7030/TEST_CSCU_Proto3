...
/******************************************************
说明：    为AES加、解密设置密钥
入口参数：keysize:密钥长度（字节）,keyBytes:密钥
出口参数：无
全局变量: 
返回值：  CRC校验值
******************************************************/
...
void SubWord(unsigned char* wd)
{
	...
	for(j=0;j<4;j++)
	{
		temp[j] = AesSbox[16*(wd[j] >> 4)+(wd[j] & 0x0f)];  //实际上也可以写成AesSbox[[j]];因为两者相等
	}
	memcpy(wd,temp,4);
...
///******************************************************
//说明：  数据的AES加、解密  
//入口参数：input:待加、解密的数据，len:参与计算的数据长度，
//is_encrypt:1-加密，0-解密
//出口参数：output,处理后返回的数据
//全局变量: 
//返回值：  加、解密后的数据长度
//******************************************************
int AesEncrypt(unsigned char* input,unsigned int iDataLen,unsigned char *psPK ,unsigned char* output,int iType)
{
	Aes(16,psPK);
    if(iType==1)
	 return AesDeEncrypt(input,iDataLen,output,1);
    else
     return AesDeEncrypt(input,iDataLen,output,0);
}
int AesDeEncrypt(unsigned char * input,int len,unsigned char* output,char is_encrypt)
{
	...
	{
		if(is_encrypt)
			Cipher(&input[i*16],&output[i*16]);
		else
			InvCipher(&input[i*16],&output[i*16]);
	}
	if(!is_encrypt)//解密
		return len;
	ifree=len%16;	
	if(ifree) //剩余不满16字节
	{
		memcpy(buf,&input[n*16],ifree);
		memset(&buf[ifree],16-ifree,16-ifree);
	}
	else //没有剩余
		memset(buf,16,16);
	//if(is_encrypt)
	Cipher(buf,&output[n*16]);
	//		else
	//			InvCipher(buf,&output[n*16]);
	return (n+1)*16;
}
////////////////////////////////////////////////////////////////////////////////////////////////
//轮密钥加
...
void DataToStr(unsigned char *psData,int iLen,unsigned char *psOut)
{
   ...
   {
	  // char cLow = psData[i]&0xF0>>4;
	  // char cUp = psData[i]&0x0F;
      // psOut[i*2+0] = ChartoStr(cLow);
      // psOut[i*2+1] = ChartoStr(cUp);
	  sprintf((char *)&psOut[i*2],"%02X",psData[i]);
   }
}
